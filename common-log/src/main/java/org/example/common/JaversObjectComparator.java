...
public class JaversObjectComparator {
    ...
    public static <T, K, R extends ListChange> List<R> extractChanges(T source, T target, Function<T, K> propExtract, Class<R> clazz) {
        // 确保源对象和目标对象不为 null
        Assert.notNull(source, "source must not be null");
        Assert.notNull(target, "target must not be null");
        ...
        return extractChanges(oldObj, newObj).stream()
                .filter(clazz::isInstance)
                .map(change -> (R) change).toList();
    }
}
